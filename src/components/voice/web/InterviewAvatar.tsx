  ...
  return (
    <div className="flex justify-center mb-4 pt-4">
      <div className="relative">
        ...
        {!isInterviewActive && (
          <>
            <button
              ...
              className={`absolute left-[-80px] md:left-[-160px] top-1/2 -translate-y-1/2 z-10 w-8 h-8 rounded-full flex items-center justify-center border-none bg-transparent hover:bg-muted transition-all duration-200 hover:scale-110 active:scale-95 ${isTransitioning ? 'opacity-50 cursor-not-allowed' : ''
                }`}
            >
            ...
            <button
              onClick={handleNext}
              disabled={isTransitioning}
              className={`absolute right-[-80px] md:right-[-160px] top-1/2 -translate-y-1/2 z-10 w-8 h-8 rounded-full flex items-center justify-center border-none bg-transparent hover:bg-muted transition-all duration-200 hover:scale-110 active:scale-95 ${isTransitioning ? 'opacity-50 cursor-not-allowed' : ''
                }`}
            >
              <svg className="w-4 h-4 text-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className={`relative transition-all duration-700 ease-in-out ${isInterviewActive
            ? isPlayingTTS
              ? 'scale-150'
              : isRecordingActive
                ? 'scale-130'
          ...
          <div className={`transition-all duration-300 ease-out ${isTransitioning
            ...
            <Image
              ...
              className={`w-40 h-40 md:w-52 md:h-52 lg:w-60 lg:h-60 object-contain transition-all duration-300 ${isInterviewActive
                  ? isPlayingTTS
                    ? ''
        ...
        {isInterviewActive && (
          <div className={`absolute inset-0 rounded-full transition-all duration-300 ${isPlayingTTS
        ...
        {isInterviewActive && (
          <div className="absolute -bottom-4 left-1/2 transform -translate-x-1/2">
            <div className={`px-3 py-1 rounded-full text-xs font-medium transition-all duration-300 ${isPlayingTTS
